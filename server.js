...
io.on('connection', socket => {                               //Функция выполняется при подкл к сокету
     socket.on('ROOM:JOIN', ({roomId, userName}) => {         //Когда приходит сокет запрос, выполнится стрелочная фция(получает информацию)
      socket.join(roomId);
      rooms.get(roomId).get('users').set(socket.id, userName);   //Подключаемся к сокету сохраняем данные в БД
      const users = [...rooms.get(roomId).get('users').values()];          //Получаем список всех пользователей 
      socket.to(roomId).emit('ROOM:JOINED', users);
           //Оповещение всех пользователей в определенной комнате кроме меня о входе с пом сокет запроса|ЛОМАЕТСЯ С BROADCAST?? 
     });
